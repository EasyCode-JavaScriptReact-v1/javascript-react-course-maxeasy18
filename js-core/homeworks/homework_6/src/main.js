...
function countLetterA(str) {
  return str.split('').reduce(function(accumulator,value){
    return accumulator + (value === 'a' ? 1 : 0);      
  },0);
}

console.log('task 1:')
console.log(randomString , '=>' ,countLetterA(randomString));
console.log(user.name + javaScript.html , '=>' ,countLetterA(user.name + javaScript.html));
...
function reverseEachWord(sentence, direction) {
  let sentenceArr = sentence.split(' ');
  // let revercedWords = []

  let revercedWords = sentenceArr.map(function(currentValue,index,arr){    
  ...
  if(!direction){
    ...
    return revercedWords.reverse().join(' ');    
  }
}

console.log('\ntask 2:');
console.log('You don\'t have to do anything special to begin using the DOM. Different browsers have different implementations of the DOM' , ' => ', reverseEachWord('You don\'t have to do anything special to begin using the DOM. Different browsers have different implementations of the DOM'));
...
console.log('The Document Object Model (DOM) is a programming interface for HTML and XML documents', '=> ', reverseEachWord('The Document Object Model (DOM) is a programming interface for HTML and XML documents'));
...
function createHashTags(arr) {
  ...
  return arr.map(function(value){
      let newObj = {};
      newObj[value._id] = value.company;
      return newObj;
...
function uniqueElements(arr) {
  ...
  arr.forEach(function(currentValue,index,arr){
    _tempObj[currentValue] = !_tempObj[currentValue] ? 1 : ++_tempObj[currentValue]; 
  });
